...
\]
where $\omega=\sqrt{\kappa/m}$ and $\kappa$ is the spring constant for the oscillators.
...
\begin{theorem}[Equipartion Theorem]
    Assume that:
    \begin{itemize}
...
\begin{proof}
    Exercise. \textbf{Hint: } Start with $\langle\alpha x^2\rangle = \frac{\int \mathrm{d}p\mathrm{d}q \exp(-\beta H)\alpha x^2}{\int \mathrm{d}p\mathrm{d}q \exp(-\beta H)}$
\end{proof}
...
\[
	Z_1 = \sum_{n=0,1,2,\ldots}\exp(-\beta H) =  \sum_{n=0,1,2,\ldots}\exp(-\beta(n+\frac12\hbar\omega)) = \frac{\exp(-\beta\hbar\omega/2))}{1-\exp(-\beta\hbar\omega))}
\] 
...
\begin{equation}
	...
	\label{eq:7.2}
\end{equation}
(We are going to assume throughout this section that all the vectors/wave functions that we deal with have already been normalised.) Now, if $\phi_a$ is any orthonormal basis then the identity operator (in some texts, and in 703, this is called the \textit{closure} or completeness relation) is
\begin{equation}
	1 = \sum_a\ket{\phi_a}\bra{\phi_a}
\end{equation}
